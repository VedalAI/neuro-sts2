...
public class CardSelectionHandler : IContextHandler
{
    ...
    private async Task<ExecutionResult> SelectCard(JsonElement root, ContextInfo ctx)
    {
    ...
    private async Task<ExecutionResult> SelectMultipleCards(ContextInfo ctx, JsonElement root)
    {
        ...
        foreach (var item in root.EnumerateArray())
        {
            ...
            var holders = ctx.CardHolders;
            if (holders == null)
                return ExecutionResult.Failure($"Card index {cardName} out of range (available: {holders?.Count ?? 0})");

            var holder = holders.First((x) => x.CardNode.Model.Title == cardName && !all_selected_nodes.Contains(x));
            if (holder == null)
            {
                return ExecutionResult.Failure($"Card name {cardName} not in deck");
            }
