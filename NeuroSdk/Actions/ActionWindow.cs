...
namespace NeuroSdk.Actions
{
    ...
    /// </summary>
    public sealed class ActionWindow : Node
    {
        #region Creation
        ...
        private bool ValidateFrozen()
        {
            ...
            {
                Plugin.LogError("Cannot mutate ActionWindow after it has been registered.");
                return false;
            }
        ...
        public void Register()
        {
            if (CurrentState != State.Building)
            {
                Plugin.LogError("Cannot register an ActionWindow multiple times.");
                return;
            }

            if (_actions.Count == 0)
            {
                Plugin.LogError("Cannot register an ActionWindow with no actions.");
                return;
            }
        ...
        public ActionWindow AddAction(INeuroAction action)
        {
            ...
            if (action.ActionWindow != null)
            {
                ...
                {
                    Plugin.LogError($"Cannot add action {action.Name} to this ActionWindow because it is already included in another ActionWindow.");
                }
            ...
            {
                Plugin.LogError($"Cannot add two actions with the same name to the same ActionWindow. Triggered by {action.Name}");
                return this;
            }
