...
public class MainThreadUtil : Node
{
    ...
    {
        // Godot's equivalent to SynchronizationContext.Post
        if (Engine.GetMainLoop() is SceneTree tree)
        {
            tree.CallDeferred(nameof(ExecuteAction), Callable.From(action));
        }
    }

    private void ExecuteAction(Callable callable)
    {
        callable.Call();
    }
}
...
{
    // Return false immediately to wait exactly one frame
